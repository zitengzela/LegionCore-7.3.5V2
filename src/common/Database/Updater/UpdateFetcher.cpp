...
void UpdateFetcher::FillFileListRecursively(Path const& path, LocaleFileStorage& storage, State const state, uint32 const depth) const
{
    ...
    for (directory_iterator itr(path); itr != end; ++itr)
    {
        ...
        {
            TC_LOG_TRACE(LOG_FILTER_UPDATE_SQL, "Added locale file \"%s\".", itr->path().filename().generic_string().c_str());
            ...
            {
                TC_LOG_FATAL(LOG_FILTER_UPDATE_SQL, "Duplicate filename \"%s\" occurred. Because updates are ordered " \
...
UpdateFetcher::DirectoryStorage UpdateFetcher::ReceiveIncludedDirectories() const
{
    ...
    do
    {
        ...
        if (!is_directory(p))
        {
            TC_LOG_WARN(LOG_FILTER_UPDATE_SQL, "DBUpdater: Given update include directory \"%s\" does not exist, skipped!", p.generic_string().c_str());
            continue;
        }
        ...
        TC_LOG_TRACE(LOG_FILTER_UPDATE_SQL, "Added applied file \"%s\" from remote.", p.filename().generic_string().c_str());
...
std::string UpdateFetcher::ReadSQLUpdate(boost::filesystem::path const& file) const
{
    ...
    if (!in.is_open())
    {
        TC_LOG_FATAL(LOG_FILTER_UPDATE_SQL, "Failed to open the sql update \"%s\" for reading! "
                                   ...
                                   int32 const cleanDeadReferencesMaxCount) const
{
    ...
    {
        TC_LOG_DEBUG(LOG_FILTER_UPDATE_SQL, "Checking update \"%s\"...", availableQuery.first.filename().generic_string().c_str());
        ...
        if (iter != applied.end())
        {
            ...
            if (!redundancyChecks)
            {
                TC_LOG_DEBUG(LOG_FILTER_UPDATE_SQL, ">> Update is already applied, skipping redundancy checks.");
                applied.erase(iter);
                continue;
            }
            ...
            {
                TC_LOG_DEBUG(LOG_FILTER_UPDATE_SQL, ">> Update is archived and marked as archived in database, skipping redundancy checks.");
                applied.erase(iter);
                continue;
            }
        }
        ...
        if (iter == applied.end())
        {
            ...
            if (hashIter != hashToName.end())
            {
                ...
                {
                    TC_LOG_WARN(LOG_FILTER_UPDATE_SQL, ">> It seems like the update \"%s\" \'%s\' was renamed, but the old file is still there! " \
                ...
                {
                    TC_LOG_INFO(LOG_FILTER_UPDATE_SQL, ">> Renaming update \"%s\" to \"%s\" \'%s\'.",
            ...
            {
                TC_LOG_INFO(LOG_FILTER_UPDATE_SQL, ">> Applying update \"%s\" \'%s\'...",
                    availableQuery.first.filename().string().c_str(), hash.substr(0, 7).c_str());
            }
        }
        ...
        else if (allowRehash && iter->second.hash.empty())
        {
            ...
            TC_LOG_INFO(LOG_FILTER_UPDATE_SQL, ">> Re-hashing update \"%s\" \'%s\'...", availableQuery.first.filename().string().c_str(),
        ...
        else
        {
            ...
            {
                TC_LOG_INFO(LOG_FILTER_UPDATE_SQL, ">> Reapplying update \"%s\" \'%s\' -> \'%s\' (it changed)...", availableQuery.first.filename().string().c_str(),
            ...
            else
            {
                ...
                {
                    TC_LOG_DEBUG(LOG_FILTER_UPDATE_SQL, ">> Updating the state of \"%s\" to \'%s\'...",
                ...
                TC_LOG_DEBUG(LOG_FILTER_UPDATE_SQL, ">> Update is already applied and matches the hash \'%s\'.", hash.substr(0, 7).c_str());
    ...
    if (!applied.empty())
    {
        ...
        for (auto const& entry : applied)
        {
            TC_LOG_WARN(LOG_FILTER_UPDATE_SQL, ">> The file \'%s\' was applied to the database, but is missing in" \
                " your update directory now!", entry.first.c_str());

            if (doCleanup)
                TC_LOG_INFO(LOG_FILTER_UPDATE_SQL, "Deleting orphaned entry \'%s\'...", entry.first.c_str());
        }
        ...
        {
            TC_LOG_ERROR(LOG_FILTER_UPDATE_SQL, "Cleanup is disabled! There were  " SZFMTD " dirty files applied to your database, " \
