...
bool DBUpdaterUtil::CheckExecutable()
{
    ...
    if (!exists(exe))
    {
        ...
        TC_LOG_FATAL(LOG_FILTER_UPDATE_SQL, "Didn't find any executable MySQL binary at \'%s\' or in path, correct the path in the *.conf (\"MySQLExecutable\").",
...
{
    TC_LOG_INFO(LOG_FILTER_UPDATE_SQL, "Database \"%s\" does not exist, do you want to create it? [yes (default) / no]: ",
    ...
    TC_LOG_INFO(LOG_FILTER_UPDATE_SQL, "Creating database \"%s\"...", pool.GetConnectionInfo().database.c_str());
    ...
    if (!file.is_open())
    {
        TC_LOG_FATAL(LOG_FILTER_UPDATE_SQL, "Failed to create temporary query file \"%s\"!", temp.generic_string().c_str());
        return false;
    }
    ...
    {
        TC_LOG_FATAL(LOG_FILTER_UPDATE_SQL, "Failed to create database %s! Does the user (named in *.conf) have `CREATE`, `ALTER`, `DROP`, `INSERT` and `DELETE` privileges on the MySQL server?", pool.GetConnectionInfo().database.c_str());
        boost::filesystem::remove(temp);
        return false;
    }

    TC_LOG_INFO(LOG_FILTER_UPDATE_SQL, "Done.");
    boost::filesystem::remove(temp);
    return true;
}
...
bool DBUpdater<T>::Update(DatabaseWorkerPool<T>& pool)
{
    ...
    TC_LOG_INFO(LOG_FILTER_UPDATE_SQL, "Updating %s database...", DBUpdater<T>::GetTableName().c_str());

    Path const sourceDirectory(BuiltInConfig::GetSourceDirectory());

    if (!is_directory(sourceDirectory))
    {
        TC_LOG_ERROR(LOG_FILTER_UPDATE_SQL, "DBUpdater: The given source directory %s does not exist, change the path to the directory where your sql directory exists (for example c:\\source\\trinitycore). Shutting down.", sourceDirectory.generic_string().c_str());
        return false;
    }
    ...
    if (!result.updated)
        TC_LOG_INFO(LOG_FILTER_UPDATE_SQL, ">> %s database is up-to-date! %s", DBUpdater<T>::GetTableName().c_str(), info.c_str());
    else
        TC_LOG_INFO(LOG_FILTER_UPDATE_SQL, ">> Applied " SZFMTD " %s. %s", result.updated, result.updated == 1 ? "query" : "queries", info.c_str());
...
bool DBUpdater<T>::Populate(DatabaseWorkerPool<T>& pool)
{
    ...
    TC_LOG_INFO(LOG_FILTER_UPDATE_SQL, "Database %s is empty, auto populating it...", DBUpdater<T>::GetTableName().c_str());

    std::string const p = DBUpdater<T>::GetBaseFile();
    if (p.empty())
    {
        TC_LOG_INFO(LOG_FILTER_UPDATE_SQL, ">> No base file provided, skipped!");
        return true;
    }
    ...
    if (!exists(base))
    {
        switch (DBUpdater<T>::GetBaseLocationType())
        {
            ...
            {
                TC_LOG_ERROR(LOG_FILTER_UPDATE_SQL, ">> Base file \"%s\" is missing. Try fixing it by cloning the source again.",
            ...
            case LOCATION_DOWNLOAD:
            {
                ...
                TC_LOG_ERROR(LOG_FILTER_UPDATE_SQL, ">> File \"%s\" is missing, download it from \"https://github.com/AshamaneProject/AshamaneCore/releases\"" \
    ...
    // Update database
    TC_LOG_INFO(LOG_FILTER_UPDATE_SQL, ">> Applying \'%s\'...", base.generic_string().c_str());
    try
    {
        ApplyFile(pool, base);
    }
    ...
    std::string const& password, std::string const& port_or_socket, std::string const& database, Path const& path)
{
    ...
    if (ret != EXIT_SUCCESS)
    {
        TC_LOG_FATAL(LOG_FILTER_UPDATE_SQL, "Applying of file \'%s\' to database \'%s\' failed!" \
