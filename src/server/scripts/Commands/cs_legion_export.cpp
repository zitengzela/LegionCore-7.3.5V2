...
public:
	...
	bool exportWaypointData(uint32 aId, uint32 aPhaseMask, uint32 aMap, float aPosition_x, float aPosition_y, float aPosition_z)
	{
		...
		if (_Result)
		{
			...
			do {
				...
				uint32 speed = _Fields[_Index++].GetUInt32();
	...
	bool exportGossipMenuOption(uint32 aGossipMenuId)
	{
		...
		do
		{
			...
			std::string _Optiontext = _Fields[_Index++].GetString();
			...
			this->fCacheStream
				...
				<< " '" << OptionIndex << "',"
				<< " '" << _OptionNPC << "',"
				<< " '" << this->SafeSQLParams(_Optiontext) << "',"
				<< " '" << OptionBroadcastTextId << "',"
				<< " '" << OptionType << "',"
				<< " '" << OptionNpcFlag << "',"
				<< " '" << VerifiedBuild << "');\n";
	...
	bool exportCreatureEquipTemplate(uint32 aCreatureEntry)
	{
		...
		if (_Result)
		{
			...
			uint32 ItemID4 = _Fields[_Index++].GetUInt32();
			uint32 ItemID5 = _Fields[_Index++].GetUInt32();
			uint32 ItemID6 = _Fields[_Index++].GetUInt32();
			//
			// Agregar la query a la cache de almacenamiento.
			this->fCacheStream
	...
	bool exportNPCTrainer(uint32 aNPCTrainer)
	{
		...
		if (_Result)
		{
			...
			do {
				...
				//
				// Solo como comentario
				this->fCacheStream << " /*" << " Spell(" << spell << "): " /*<< GetSpellNameByEntry(SpellID)*/ << "*/\n";
	...
	bool exportCreatureOnkillReputation(uint32 aCreatureOnkillReputation)
	{
		...
		if (_Result)
		{
			...
			do {
				...
				RewFaction = int32(_Fields[_Index++].GetInt16());
				RewValue = int32(_Fields[_Index++].GetInt16());
				MaxStanding = _Fields[_Index++].GetInt8();
	...
	bool exportSkinningLootTemplate(uint32 aSkinningLootTemplate, std::ostringstream &aDescriptions)
	{
		...
		if (_Result)
		{
			...
			do {
				...
				ChanceOrQuestChance = _Fields[_Index++].GetFloat();
				LootMode = _Fields[_Index++].GetUInt16();
				GroupId = _Fields[_Index++].GetUInt8();
				mincountOrRef = _Fields[_Index++].GetInt32();
				MaxCount = _Fields[_Index++].GetUInt32();
	...
	bool exportCreatureText(uint32 aEntry)
	{
		...
		QueryResult _Result = WorldDatabase.PQuery("SELECT * FROM `creature_text` WHERE Entry = '%u';", aEntry);
	...
	bool ExportQuestToSQLFile(uint32 aEntry)
	{
		...
		if (this->exportQuestTemplate(aEntry, _QuestName))
		{
			...
			Quest const* _quest = sQuestDataStore->GetQuestTemplate(aEntry);
...
public:
	...
	static bool HandleExportPositionCommand(ChatHandler* aHandler, char const* aArgs)
	{
		...
		if (target)
		{
			...
			// Write File
			FILE* fout = fopen("Export/Positions.sql", "a");
