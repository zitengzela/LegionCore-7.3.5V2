...
struct instance_AssaultBrokenShore : public InstanceScript
{
    instance_AssaultBrokenShore(Map* map) : InstanceScript(map) {}
        ...
        switch (newStep)
        {
            ...
            case 8:
            {
                ...
                for (Map::PlayerList::const_iterator i = PlList.begin(); i != PlList.end(); ++i)
                    if (Player* player = i->getSource())
                    {
                        player->AddDelayedEvent(4000, [player, this]() -> void
                        {
                            player->CreateConversation(4892);
                            updateCriteriaForPlayers(58013);
...
{
    RegisterInstanceScript(instance_AssaultBrokenShore, 1666);
}
