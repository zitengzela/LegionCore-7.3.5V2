...
public:
	...
	struct npc_spelurk_spiritAI : public ScriptedAI
	{
		npc_spelurk_spiritAI(Creature* creature) : ScriptedAI(creature) {}
		...
		void Reset()
		{
			if (GameObject* gob = me->FindNearestGameObject(GOB_CAVE, 100.0f))
			{
				gob->RemoveFromWorld();
			}
		}
		...
		void EnterCombat(Unit* who)
		{
			GameObject* pGameObj = sObjectMgr->IsStaticTransport(GOB_CAVE) ? new StaticTransport : new GameObject;
			G3D::Quat quat(G3D::Matrix3::fromEulerAnglesZYX(5.68f, 0.f, 0.f));
			if (!pGameObj->Create(sObjectMgr->GetGenerator<HighGuid::GameObject>()->Generate(), GOB_CAVE, me->GetMap(), me->GetPhaseMask(), Position(-545.786f, -5488.85f, 7.58f, 5.68f), quat, 100.0f, GO_STATE_ACTIVE))
			{
				delete pGameObj;
				return;
			}
			pGameObj->SetRespawnTime(3600);
			me->GetMap()->AddToMap(pGameObj);
	...
	CreatureAI* GetAI(Creature* creature) const
	{
		return new npc_spelurk_spiritAI(creature);
	}
};

// Gameobject id - 220815 Cave-in
class cave_in : public GameObjectScript
{
public:
	enum eSpells
	{
		ROCK_BREAKING_HAMMER = 144393,
		BREAK_ROCKS = 144388,
	};

	cave_in() : GameObjectScript("cave_in") { }

	bool OnGossipHello(Player* player, GameObject* gameobject) override
	{
		// if caster has aura can destroy the gameobject
		if (player->HasAura(ROCK_BREAKING_HAMMER))
		{
			player->CastSpell(gameobject, BREAK_ROCKS, true);
